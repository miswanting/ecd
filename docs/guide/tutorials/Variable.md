...
写完之后，我们和游戏引擎都会产生一个疑问：”究竟哪个3是金币的数量，而哪个3是苹果的数量呢？“

别慌，显示的值不可名状，但我们可以用一种”有名字的容器“来代替：
...
接下来，我们加点难度，假如我的苹果后来变成了2个，该怎么表示？

在上面的石头苹果的例子中，在学会变量这个概念之前，我们和游戏引擎都不知道应该把哪个`3`变成`2`，但既然现在我们学会了**变量**这个概念，我们就可以做到这一点。

过程很简单，只需要告诉引擎，我的苹果的数量变为了`2`就可以：
...
完美！程序正确地理解了我们的意思，并且把两个`3`正确地区分了开来！不仅可以设置新的值，我们还可以对他进行各种操作和运算，但那都是后文了，至少现在，我们可以知道，”有名字的容器“——**变量**真的很方便！
...
好了，我们现在已经有一个方法实现不那么“直接”的数据暂存了，但仍然不能够通过改变数据来为游戏增加变化。具体地说，如果我们就是想改变`Food`的值，怎么办？
