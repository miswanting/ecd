...
Emuera 1.712新增了新的语法，被称为**表达式内函数**，或者叫“方法”（Method）。

ERB脚本中，**函数**是指以 `@ ~ ~` 的形式定义，通过`CALL`命令来调用的东西。
而**表达式内函数**则是同样以 `@ ~ ~` 形式定义，但直接在表达式中使用的东西。
简单的说，表达式内函数是可以在表达式中使用的**函数**。
