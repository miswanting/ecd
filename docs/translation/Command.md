...
第 2 参数是一个字符串，用于指定格式化数值的格式，与 `TOSTR <数式>, <格式指示符>` 指令的参数类似。
...
当起始位置或结束位置无法在字符边界切开时（即指向全角字符中间），会被判定为向后偏移 1 个位置，因此可能返回比指定字符数多 1 个字符的字符串，请注意。
...
分割后元素数超过第 3 参数可代入数量的部分不会被赋值。`RESULT` 中存入的是实际的分割数，请据此判断。
...
`WHILE`～`WEND` 是循环语法：只要 `<数式>` 不为 0 就重复循环。
...
只要 `<数式>` 不为 0 就重复循环。与 `WHILE`～`WEND` 不同，其特点是至少会执行一次。
...
- `IS <运算符> <数式>`：例如 `IS <= 30`，则 X 为 30 以下时执行该 `CASE` 以下。
- `<数式> TO <数式>`：例如 `10 TO 20`，则 X 为 10 以上 20 以下时执行该 `CASE` 以下。
...
注意 `IS` 和 `TO` 必须写成 `IS <运算符> <数式>`、`<数式> TO <数式>` 的形式。不能写成 `30 < IS` 或 `(10 TO 20) || (30 TO 40)` 这样。

另外，`<数式> TO <数式>` 仅在左边以上、右边以下时为真。右边小于左边时，该 `CASE` 永远不会被执行。
...
`SELECTCASE` 的分支条件。可用 `<值>`、`<起始值> TO <结束值>`、`IS <运算符> <数式>` 以及用逗号分隔的多个条件。
### CASEELSE
...
事先把命令编号代入 `SELECTCOM:(1～)`，并以要执行的命令数量作为参数执行。
...
参数指定的编号对应 `train.csv` 中定义的编号。动作与命令被选中时相同：初始化 `UP`、`DOWN` 等变量，把参数代入 `SELECTCOM`，调用 `@EVENTCOM`，调用 `@COM{SELECTCOM}`……
...
虽然目标是式中函数，但请用普通函数的参数格式调用。返回值会被丢弃。

当然，除非被调用的式中函数内部修改了 `RESULT` 或 `RESULTS`，否则它们不会变化。
### CALLFORMF `<FORM格式文本>` (, 参数1, 参数2……)

`CALLF` 的带格式字符串版，可以用带格式的字符串指定式中函数名。
...
把参数指定的带格式字符串作为数式解析，然后执行 `RETURN`。
...
客户端区域的大小可以用 `CLIENTWIDTH`、`CLIENTHEIGHT` 函数获取。如果需要以客户端区域左上角为基准的 Y 坐标，可以用 `MOUSEY() + CLIENTHEIGHT()` 取得。
...
精灵的尺寸可以用 `SPRITEWIDTH(str imgName)`、`SPRITEHEIGHT(str imgName)` 函数取得。
...
指定 ID 的 Graphics 已创建时取得 `1`，未创建（包括已废弃）时取得 `0`。

### GWIDTH `<ID>`

（※仅当绘制方式为 `GRAPHICS` 或 `TEXTRENDERER` 时可用）

取得指定 ID 的 Graphics 的宽度。未创建（包括已废弃）时返回 0。

### GHEIGHT `<ID>`

（※仅当绘制方式为 `GRAPHICS` 或 `TEXTRENDERER` 时可用）

取得指定 ID 的 Graphics 的高度。未创建（包括已废弃）时返回 0。

### GGETCOLOR `<ID>`, `<x>`, `<y>`

（※仅当绘制方式为 `GRAPHICS` 或 `TEXTRENDERER` 时可用）

以 `0xAARRGGBB` 形式的整数值取得指定 ID 的 Graphics 中指定位置的颜色。

Graphics 未创建或已废弃，或者 x、y 在图像之外时返回 `-1`。

请注意，只有该指令在失败时返回 `-1` 而不是 0。当取得黑色且完全透明位置的颜色时，该指令返回 0。
...
以 `0xAARRGGBB` 形式的整数值取得 `spriteName` 指定资源名的精灵中指定位置的颜色。
...
取得指定名称的精灵的宽度。精灵未创建或已废弃时返回 0。

### SPRITEHEIGHT `<精灵名>`

取得指定名称的精灵的高度。精灵未创建或已废弃时返回 0。

### SPRITEPOSX `<精灵名>`

取得指定名称的精灵的相对位置的 X。精灵未创建或已废弃时返回 0。

要区分相对位置 X 为 0 与未创建或已废弃，请另外调用 `SPRITECREATED`。

### SPRITEPOSY `<精灵名>`

取得指定名称的精灵的相对位置的 Y。精灵未创建或已废弃时返回 0。
